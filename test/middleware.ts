...
import {parseSnakeCaseToCamelCase} from "../src/api";
...
test("validateAuthHeaderAndGetUser rejects expired access tokens", async () => {
    ...
    await expect(validateAuthHeaderAndGetUser(authHeader))
        .rejects
...
test("validateAuthHeaderAndGetUser rejects missing authorization header", async () => {
    ...
    await expect(validateAuthHeaderAndGetUser(null))
        .rejects
        .toThrow(UnauthorizedException)
})
...
test("validateAuthHeaderAndGetUserWithOrgInfo get user and org for extracted org", async () => {
    const {privateKey, publicKey} = await generateRsaKeyPair()
    const {validateAuthHeaderAndGetUserWithOrgInfo} = initAuth({authUrl: AUTH_URL + "/", verifierKey: publicKey, apiKey: ""})
...
test("validateAuthHeaderAndGetUserWithOrgInfo fails for valid access token but unknown org", async () => {
    ...
    await expect(validateAuthHeaderAndGetUserWithOrgInfo(`Bearer ${accessToken}`, orgInfo))
        .rejects
...
test("validateAuthHeaderAndGetUserWithOrgInfo fails for invalid access token", async () => {
    ...
    await expect(validateAuthHeaderAndGetUserWithOrgInfo(null, orgInfo))
        .rejects
        .toThrow(UnauthorizedException)
})
...
test("validateAccessTokenAndGetUserWithOrgInfoWithMinimumRole works with miniumumRole", async () => {
    const {privateKey, publicKey} = await generateRsaKeyPair()
    const {validateAuthHeaderAndGetUserWithOrgInfoWithMinimumRole} = initAuth({authUrl: AUTH_URL + "/", verifierKey: publicKey, apiKey: ""})
    ...
    for (let role of ["Owner", "Admin", "Member"]) {
        ...
        if (rolesThatShouldSucceed.has(role)) {
            const userAndOrgMemberInfo = await validateAuthHeaderAndGetUserWithOrgInfoWithMinimumRole(authHeader, orgInfo, role)
            expect(userAndOrgMemberInfo.user).toEqual(user)
            expect(userAndOrgMemberInfo.orgMemberInfo).toEqual(user.orgIdToOrgMemberInfo && user.orgIdToOrgMemberInfo[orgMemberInfo.org_id])
        } else {
            await expect(validateAuthHeaderAndGetUserWithOrgInfoWithMinimumRole(authHeader, orgInfo, role))
                .rejects
                .toThrow(ForbiddenException)
        }
    }
...
test("validateAuthHeaderAndGetUserWithOrgInfoWithExactRole works with requiredRole", async () => {
    const {privateKey, publicKey} = await generateRsaKeyPair()
    const {validateAuthHeaderAndGetUserWithOrgInfoWithExactRole} = initAuth({authUrl: AUTH_URL + "/", verifierKey: publicKey, apiKey: ""})
    ...
    for (let role of ["Owner", "Admin", "Member"]) {
        ...
        if (rolesThatShouldSucceed.has(role)) {
            const userAndOrgMemberInfo = await validateAuthHeaderAndGetUserWithOrgInfoWithExactRole(authHeader, orgInfo, role)
            expect(userAndOrgMemberInfo.user).toEqual(user)
            expect(userAndOrgMemberInfo.orgMemberInfo).toEqual(user.orgIdToOrgMemberInfo && user.orgIdToOrgMemberInfo[orgMemberInfo.org_id])
        } else {
            await expect(validateAuthHeaderAndGetUserWithOrgInfoWithExactRole(authHeader, orgInfo, role))
                .rejects
                .toThrow(ForbiddenException)
        }
    }
...
test("validateAuthHeaderAndGetUserWithOrgInfoWithPermission works with permission", async () => {
    const {privateKey, publicKey} = await generateRsaKeyPair()
    const {validateAuthHeaderAndGetUserWithOrgInfoWithPermission} = initAuth({authUrl: AUTH_URL + "/", verifierKey: publicKey, apiKey: ""})
    ...
    for (let permission of ["read", "write", "delete"]) {
        ...
        if (permissionsThatShouldSucceed.has(permission)) {
            const userAndOrgMemberInfo = await validateAuthHeaderAndGetUserWithOrgInfoWithPermission(authHeader, orgInfo, permission)
            expect(userAndOrgMemberInfo.user).toEqual(user)
            expect(userAndOrgMemberInfo.orgMemberInfo).toEqual(user.orgIdToOrgMemberInfo && user.orgIdToOrgMemberInfo[orgMemberInfo.org_id])
        } else {
            await expect(validateAuthHeaderAndGetUserWithOrgInfoWithPermission(authHeader, orgInfo, permission))
                .rejects
                .toThrow(ForbiddenException)
        }
    }
...
test("validateAuthHeaderAndGetUserWithOrgInfoWithAllPermissions works with permission", async () => {
    const {privateKey, publicKey} = await generateRsaKeyPair()
    const {validateAuthHeaderAndGetUserWithOrgInfoWithAllPermissions} = initAuth({authUrl: AUTH_URL + "/", verifierKey: publicKey, apiKey: ""})
    ...
    for (let permissions of [["read"], ["write"], ["read", "write"], []]) {
        const authHeader = `Bearer ${accessToken}`
        const userAndOrgMemberInfo = await validateAuthHeaderAndGetUserWithOrgInfoWithAllPermissions(authHeader, orgInfo, permissions)
        expect(userAndOrgMemberInfo.user).toEqual(user)
        expect(userAndOrgMemberInfo.orgMemberInfo).toEqual(user.orgIdToOrgMemberInfo && user.orgIdToOrgMemberInfo[orgMemberInfo.org_id])
    }
    ...
    for (let permissions of [["read", "write", "delete"], ["delete"]]) {
        const authHeader = `Bearer ${accessToken}`
        await expect(validateAuthHeaderAndGetUserWithOrgInfoWithAllPermissions(authHeader, orgInfo, permissions))
            .rejects
            .toThrow(ForbiddenException)
    }
})
...
async function createAccessToken({internalUser, privateKey, expiresIn, issuer}: CreateAccessTokenArgs): Promise<string> {
